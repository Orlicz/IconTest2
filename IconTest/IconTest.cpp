...
const auto taskWin = FindWindow(L"Shell_TrayWnd", NULL);
...
wchar_t* strBuffer = new wchar_t[256];
inline auto UserName() {
	DWORD dwSize = 256;
	...
	_In_ int nCmdShow)
{
	...
	constexpr int MaxRun = 126;
	constexpr int Trans = 127;
	...
	RegisterHotKey(NULL, MaxRun, MOD_ALT, 'O');
	RegisterHotKey(NULL, Trans, MOD_ALT, 'T');
	...
	while (GetMessage(&msg, nullptr, 0, 0)) {
		if (msg.message == WM_HOTKEY) {
			...
			case Trans: {
				if (Iputs) {
					auto* http = new HttpConnect<bufer>();

					char bufferch[100];
					GetWindowTextA(Iputs, bufferch, sizeof(bufferch) / sizeof(bufferch[0]));
					std::string buffer(bufferch);
					
					
					replace_all(buffer, "\r\n", "   ");
					auto InputsArr = stringSplit(buffer, "\r\n");
					std::string thra;
					int resitsz = InputsArr.end() - InputsArr.begin();

					for (auto& mybf : InputsArr) {
						std::string gets = "keyfrom=xujiangtao&key=1490852988&type=data&doctype=json&version=1.1&q=";

						gets = gets + buffer;
						auto res = utf8_to_ansi(http->getData("fanyi.youdao.com", "/openapi.do", gets));

						std::regex pattern("\\[([^\\]]+)\\]");
						auto match_iter = std::sregex_iterator(res.begin(), res.end(), pattern);
						...
						if (resitsz != 1) {
							...
							for (; match_iter != match_end; match_iter++)
							{
								...
								thra += m[1];   // sub-match for token in parentheses, the 1079, 1165, ...
								thra += "\r\n";										// m[0]: whole match, "1079//"
																		// m[1]: first submatch, "1070"
							}
						}
					}
					SetWindowTextA(Iputs, thra.c_str());
				}
				break;
			}
			case SrePic: {
			...
			case MaxRun: {
				if (opend) {
					ShowWindow(taskWin, SW_SHOW);//显示
					ShowWindow(DeskTopWin, SW_SHOW);//显示
					SetWindowPos(GetForegroundWindow(), HWND_NOTOPMOST, ct.left, ct.top, ct.right - ct.left, ct.bottom - ct.top, SWP_SHOWWINDOW);
				}
				else {
					ShowWindow(taskWin, SW_HIDE);//隐藏任务栏
					ShowWindow(DeskTopWin, SW_HIDE);//隐藏桌面

					FullScreen(GetForegroundWindow(), ct);
				}
				opend = !opend;
				break;
			}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message) {
		ResNotify(*Myn) {
			if (lParam == 1029) {
				ShellExecute(NULL, L"open", (L"C:\\Users\\" + std::wstring(myUserName()) + L"\\Pictures\\PrintScreen").c_str(), NULL, NULL, SW_SHOWDEFAULT);
			}
			switch (lParam) {
