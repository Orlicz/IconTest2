...
void FullScreen(HWND hwnd, RECT& old) {
	HWND    hDesk;
	RECT    rc;
...
{
	m_Notify.uFlags = NIF_MESSAGE | NIF_INFO | NIF_TIP;
...
#pragma region MyRegion
auto OnBnClickedButton1(HWND Wnd)
{
	auto* a = new NOTIFYICONDATA;
	auto nid = *a;
	// TODO: 在此添加控件通知处理程序代码   
		...
		nid.uCallbackMessage = IDM_OPENPIC;//自定义的消息名称,注意:这里的消息是用户自定义消息  
	...
	AnimateWindow(Wnd,2000,AW_HIDE|AW_BLEND);      //可以缓慢消失窗口  
   // KillTimer(0);  
	ShowWindow(Wnd,SW_HIDE);//隐藏主窗口  
	return a;
}
#pragma endregion
